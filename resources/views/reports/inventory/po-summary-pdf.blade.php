...
<body>
    <header>
        ...
        <div class="header-subtitle">Purchase Order Summary Report - {{ $validated['date_from'] }} to {{ $validated['date_to'] }}</div>
    </header>
    ...
    <div class="summary">
        <strong>Summary:</strong> Total POs: {{ $summary['total_pos'] }}, Total Amount: ₱{{ number_format($summary['total_amount'], 2) }}, Completed: {{ $summary['completed'] }}, Pending: {{ $summary['pending'] }}
    </div>
    ...
    <table>
        ...
        <tbody>
            @foreach($purchaseOrders as $po)
                <tr>
                    <td>PO #{{ str_pad($po->POID, 4, '0', STR_PAD_LEFT) }}</td>
                    <td>{{ $po->supplier->SupplierName ?? 'N/A' }}</td>
                    <td>{{ $po->OrderDate->format('M d, Y') }}</td>
                    <td>{{ $po->Status }}</td>
                    <td>₱{{ number_format($po->TotalAmount ?? 0, 2) }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
